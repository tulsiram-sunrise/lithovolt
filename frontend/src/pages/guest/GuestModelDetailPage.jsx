...
export default function GuestModelDetailPage() {
  const { id } = useParams()

  const { data, isLoading, isError } = useQuery({
    queryKey: ['guest-model-detail', id],
    queryFn: () => publicCatalogAPI.getBatteryModels({ per_page: 300 }),
  ...
  return (
    <div className="space-y-6 md:space-y-7">
      ...
      {!isLoading && !isError && !model ? (
        <div className="panel-card p-6">
          <p className="text-[color:var(--muted)]">Model not found.</p>
        </div>
      ) : null}
