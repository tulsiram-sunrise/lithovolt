# Terms of Service for Lithovolt Battery Manager

**Effective Date**: February 19, 2026  
**Last Updated**: February 19, 2026  
**Version**: 1.0

---

## 1. Acceptance of Terms

Welcome to Lithovolt Battery Manager ("the App," "we," "us," or "our"). By accessing or using the Lithovolt Battery Manager mobile application and related services, you agree to be bound by these Terms of Service ("Terms").

**IF YOU DO NOT AGREE TO THESE TERMS, DO NOT USE THE APP.**

These Terms constitute a legally binding agreement between you ("User," "you," or "your") and Lithovolt Inc. ("Company," "we," "us," or "our").

---

## 2. Definitions

- **"Account"**: Your registered user profile on the App
- **"Content"**: All information, data, text, photographs, and materials on the App
- **"Services"**: All features and functionality provided by the App
- **"User Content"**: Content you submit, upload, or transmit through the App
- **"Warranty Certificate"**: Digital warranty document generated through the App
- **"QR Code"**: Quick Response code used for battery verification

---

## 3. User Accounts

### 3.1 Account Creation

To use certain features, you must create an account:
- Provide accurate, current, and complete information
- Maintain and update your account information
- Keep your password secure and confidential
- Notify us immediately of unauthorized access

### 3.2 Account Types

**Wholesaler/Retailer Accounts**:
- Require business verification
- Subject to approval process
- Additional terms may apply

**Consumer Accounts**:
- Personal use only
- One account per person
- May use OTP-based authentication

### 3.3 Account Responsibilities

You are responsible for:
- All activities under your account
- Maintaining confidentiality of credentials
- Compliance with these Terms
- Actions of anyone using your account

### 3.4 Account Termination

We may suspend or terminate your account if:
- You violate these Terms
- You provide false information
- Your account is used for fraudulent activities
- Required by law or regulation
- The account has been inactive for extended periods

---

## 4. Permitted Use

### 4.1 License Grant

We grant you a limited, non-exclusive, non-transferable, revocable license to:
- Download and install the App on your device
- Access and use the Services for legitimate business or personal purposes
- Generate and share warranty certificates
- Scan QR codes for verification

### 4.2 Restrictions

You agree NOT to:
- Use the App for illegal purposes
- Reverse engineer, decompile, or disassemble the App
- Copy, modify, or create derivative works
- Remove copyright or proprietary notices
- Use automated systems (bots, scrapers) without permission
- Attempt to gain unauthorized access to systems
- Impersonate others or falsify information
- Upload viruses or malicious code
- Interfere with the App's operation
- Use the App to distribute spam or unsolicited messages
- Violate intellectual property rights
- Resell or redistribute the App or Services

### 4.3 Business Use Terms

**For Wholesalers and Retailers**:
- Use only for legitimate business operations
- Verify customer information accurately
- Issue warranties only for genuine products
- Maintain accurate inventory records
- Comply with manufacturer requirements
- Respect consumer privacy and data protection laws

---

## 5. Services Description

### 5.1 Core Services

**Inventory Management**:
- Track battery models and serial numbers
- Manage stock allocations
- View inventory statistics

**Order Management**:
- Place and track orders
- Receive order updates
- Access order history

**Warranty Management**:
- Register warranties
- Generate warranty certificates
- Verify warranty status
- Submit warranty claims

**QR Code Verification**:
- Scan QR codes on batteries
- Verify product authenticity
- Access product information

### 5.2 Service Availability

- Services are provided "as is" and "as available"
- We do not guarantee uninterrupted or error-free operation
- We may modify, suspend, or discontinue features at any time
- Scheduled maintenance may cause temporary unavailability
- We will provide notice of significant changes when possible

### 5.3 Service Limitations

- Features may vary by account type
- Some services require internet connectivity
- QR scanning requires camera access and adequate lighting
- File sizes for uploads are limited
- API rate limits may apply

---

## 6. User Content

### 6.1 Your Content

You retain ownership of content you submit:
- Battery serial numbers and inventory data
- Warranty information and certificates
- Photos for warranty claims
- Profile information and business details

### 6.2 License to Use Your Content

By submitting content, you grant us:
- Non-exclusive, worldwide license to use, store, and display your content
- Right to process and analyze data for service provision
- Permission to back up your data
- Ability to share with manufacturers for warranty verification

### 6.3 Content Standards

You agree that your content will:
- Be accurate and not misleading
- Comply with applicable laws
- Not infringe third-party rights
- Not contain malicious code
- Not include inappropriate or offensive material

### 6.4 Content Removal

We reserve the right to:
- Remove content that violates these Terms
- Moderate user-generated content
- Refuse to store or transmit certain content
- Disable access to content under investigation

---

## 7. Intellectual Property

### 7.1 Our Intellectual Property

All App content, features, and functionality are owned by us and protected by:
- Copyright laws
- Trademark laws
- Patent laws
- Trade secret laws

This includes:
- App design, code, and architecture
- Lithovolt name, logo, and branding
- QR code generation algorithms
- Proprietary processes and methods

### 7.2 Trademarks

"Lithovolt," the Lithovolt logo, and other marks are our trademarks. You may not use them without written permission.

### 7.3 Copyright Infringement

If you believe content infringes your copyright:
- Email: copyright@lithovolt.com.au
- Include detailed description and proof of ownership
- We will investigate and take appropriate action

### 7.4 Third-Party Content

The App may include third-party content:
- We do not own or control third-party content
- Third-party content is subject to separate terms
- We are not responsible for third-party content

---

## 8. Payments and Fees

### 8.1 Free Services

Basic features are provided free of charge:
- Account creation
- QR code scanning
- Warranty verification
- Basic inventory tracking

### 8.2 Paid Services (if applicable)

Premium features may require payment:
- Advanced analytics
- Additional storage
- Priority support
- Extended warranty services

### 8.3 Payment Terms

If you purchase paid services:
- Prices are in USD (or local currency)
- Payment is processed by third-party providers
- All fees are non-refundable unless required by law
- Subscriptions auto-renew unless cancelled
- We may change pricing with 30 days' notice

### 8.4 Taxes

You are responsible for:
- Sales tax, VAT, or other applicable taxes
- Compliance with tax laws in your jurisdiction

---

## 9. Warranty and Disclaimer

### 9.1 Limited Warranty

**Regarding Warranty Certificates**:
- Certificates reflect data provided by users
- We are not responsible for accuracy of information
- Warranty terms are determined by battery manufacturers
- We facilitate but do not guarantee warranty claims

### 9.2 Disclaimer of Warranties

THE APP IS PROVIDED "AS IS" WITHOUT WARRANTIES OF ANY KIND:
- NO WARRANTY OF MERCHANTABILITY
- NO WARRANTY OF FITNESS FOR A PARTICULAR PURPOSE
- NO WARRANTY OF NON-INFRINGEMENT
- NO WARRANTY OF UNINTERRUPTED OR ERROR-FREE OPERATION

### 9.3 Third-Party Services

We are not responsible for:
- Third-party payment processors
- Third-party shipping or logistics
- Manufacturer warranty policies
- External websites or services

---

## 10. Limitation of Liability

### 10.1 Exclusion of Damages

TO THE MAXIMUM EXTENT PERMITTED BY LAW, WE SHALL NOT BE LIABLE FOR:
- Indirect, incidental, or consequential damages
- Loss of profits, data, or business opportunities
- Damages resulting from use or inability to use the App
- Unauthorized access to your data
- Actions of third parties

### 10.2 Liability Cap

Our total liability for all claims shall not exceed:
- The amount you paid us in the 12 months before the claim, OR
- $100 USD, whichever is greater

### 10.3 Jurisdictional Limitations

Some jurisdictions do not allow limitation of liability. In such cases, our liability is limited to the extent permitted by law.

---

## 11. Indemnification

You agree to indemnify and hold us harmless from:
- Claims arising from your use of the App
- Your violation of these Terms
- Your violation of third-party rights
- Your User Content
- Your business operations or transactions

This includes legal fees and costs of defending against such claims.

---

## 12. Dispute Resolution

### 12.1 Governing Law

These Terms are governed by the laws of [Your Jurisdiction] without regard to conflict of law provisions.

### 12.2 Informal Resolution

Before filing a claim:
- Contact us at legal@lithovolt.com.au
- Provide detailed description of dispute
- Attempt good-faith resolution for 60 days

### 12.3 Arbitration (if applicable)

Disputes may be resolved through binding arbitration:
- Conducted under [Arbitration Rules]
- In [Location]
- One arbitrator mutually agreed upon
- Award is final and binding

### 12.4 Class Action Waiver

You agree to resolve disputes individually, not as part of class or collective actions.

### 12.5 Exceptions

Certain disputes are not subject to arbitration:
- Intellectual property claims
- Small claims court matters
- Injunctive or equitable relief

---

## 13. Privacy and Data Protection

Your privacy is important to us:
- See our Privacy Policy for details on data collection and use
- Privacy Policy is incorporated by reference into these Terms
- Available at: [Privacy Policy URL]

Key points:
- We collect and use data as described in Privacy Policy
- You consent to data processing as described
- We implement security measures to protect data
- You have rights regarding your data (see Privacy Policy)

---

## 14. Communications

### 14.1 Electronic Communications

By using the App, you consent to:
- Receive electronic communications from us
- Email notifications and updates
- In-app messages and alerts
- SMS messages (if you opt in)

### 14.2 Opt-Out

You may opt out of marketing communications:
- Adjust settings in the App
- Click "unsubscribe" in emails
- Contact support@lithovolt.com.au

**Note**: You cannot opt out of transactional or administrative communications.

---

## 15. Changes to Terms

### 15.1 Right to Modify

We may modify these Terms at any time:
- Changes effective upon posting
- Continued use constitutes acceptance
- Material changes require affirmative consent
- Notice provided via email or in-app notification

### 15.2 Review Responsibility

You are responsible for:
- Reviewing Terms periodically
- Checking for updates
- Understanding current Terms

### 15.3 Right to Terminate

If you disagree with changes:
- Stop using the App
- Close your account
- Contact us with concerns

---

## 16. Termination

### 16.1 Termination by You

You may terminate at any time by:
- Deleting the App from your device
- Closing your account through App settings
- Contacting support@lithovolt.com.au

### 16.2 Termination by Us

We may terminate or suspend access:
- For violation of these Terms
- For fraudulent activity
- At our discretion with or without notice
- To comply with legal requirements

### 16.3 Effect of Termination

Upon termination:
- License to use App immediately revoked
- Your account will be deactivated
- You must cease all use of the App
- We may delete your data (subject to legal retention requirements)
- Obligations that should survive termination remain in effect

### 16.4 Survival

The following sections survive termination:
- Intellectual Property
- Disclaimer of Warranties
- Limitation of Liability
- Indemnification
- Dispute Resolution

---

## 17. General Provisions

### 17.1 Entire Agreement

These Terms, along with Privacy Policy, constitute the entire agreement between you and us regarding the App.

### 17.2 Severability

If any provision is found invalid or unenforceable:
- That provision will be modified to be enforceable
- Or removed if modification is not possible
- Remaining provisions remain in full effect

### 17.3 Waiver

Failure to enforce any right or provision does not constitute waiver of that right or provision.

### 17.4 Assignment

You may not assign or transfer these Terms without our consent. We may assign these Terms at any time.

### 17.5 Force Majeure

We are not liable for failure or delay due to circumstances beyond our reasonable control:
- Natural disasters
- War, terrorism, or civil unrest
- Government actions
- Internet or telecommunications failures
- Pandemics or public health emergencies

### 17.6 Export Controls

You agree to comply with:
- Export control laws and regulations
- Not use App in prohibited countries
- Not use App for prohibited purposes

### 17.7 Third-Party Beneficiaries

These Terms do not create rights for any third parties.

### 17.8 Headings

Section headings are for convenience only and do not affect interpretation.

---

## 18. Contact Information

For questions about these Terms:

**Email**: legal@lithovolt.com.au  
**Support**: support@lithovolt.com.au  
**Address**:  
Lithovolt Inc.  
[Street Address]  
[City, State/Province, ZIP/Postal Code]  
[Country]

**Business Hours**: Monday-Friday, 9:00 AM - 5:00 PM [Time Zone]  
**Response Time**: Within 48 hours for general inquiries

---

## 19. Special Terms for Business Users

### 19.1 Business Verification

Wholesalers and retailers must:
- Provide valid business credentials
- Undergo verification process
- Maintain current business information
- Comply with manufacturer requirements

### 19.2 Warranty Issuance Responsibilities

When issuing warranties:
- Verify product authenticity
- Collect accurate consumer information
- Follow manufacturer guidelines
- Maintain proper records
- Honor warranty terms

### 19.3 Compliance

Business users must comply with:
- Consumer protection laws
- Data protection regulations
- Industry standards
- Manufacturer policies

### 19.4 Business Account Termination

Additional grounds for business account termination:
- Fraudulent warranty issuance
- Counterfeit product distribution
- Repeated customer complaints
- Failure to maintain business credentials

---

## 20. Mobile App Specific Terms

### 20.1 App Stores

The App is available through third-party app stores:
- Apple App Store
- Google Play Store

You agree to comply with applicable app store terms.

### 20.2 Updates

We may release updates:
- Bug fixes and improvements
- New features
- Security patches
- Operating system compatibility

You should install updates promptly for best experience and security.

### 20.3 Permissions

The App requires certain device permissions:
- Camera (for QR scanning)
- Storage (for certificate downloads)
- Notifications (for alerts)
- Location (optional, for service center finder)

You may revoke permissions in device settings, but some features may not function.

### 20.4 Device Requirements

Minimum requirements:
- iOS 12.0 or later / Android 8.0 or later
- Internet connection
- Camera for QR scanning
- Sufficient storage space

---

## Acknowledgment

By clicking "Accept," creating an account, or using the App, you acknowledge that:
1. You have read these Terms
2. You understand these Terms
3. You agree to be bound by these Terms
4. You are of legal age to enter into this agreement
5. If using on behalf of a business, you have authority to bind that business

---

**END OF TERMS OF SERVICE**

*These Terms are effective as of the date indicated above. Please review regularly for updates.*

---

**Document Version**: 1.0  
**Last Updated**: February 19, 2026  
**Next Review**: Annually or as needed

*This is a template. Consult with a legal professional to ensure compliance with all applicable laws before publishing.*
