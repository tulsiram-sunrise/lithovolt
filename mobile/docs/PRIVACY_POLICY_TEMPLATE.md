# Privacy Policy for Lithovolt Battery Manager

**Effective Date**: February 19, 2026  
**Last Updated**: February 19, 2026  
**Version**: 1.0

---

## Introduction

Welcome to Lithovolt Battery Manager ("we," "our," or "us"). We are committed to protecting your privacy and ensuring the security of your personal information. This Privacy Policy explains how we collect, use, disclose, and safeguard your information when you use our mobile application and related services.

By using Lithovolt Battery Manager, you agree to the collection and use of information in accordance with this policy. If you do not agree with our policies and practices, please do not use our application.

---

## 1. Information We Collect

### 1.1 Information You Provide Directly

**Account Information**:
- Full name
- Email address
- Phone number
- Company name and address (for business accounts)
- Role/user type (wholesaler, retailer, consumer)
- Password (encrypted and never stored in plain text)

**Business Information** (for wholesalers and retailers):
- Business license number
- Tax identification number
- Business address and contact details
- Bank account information (for payment processing)

**Transaction Information**:
- Battery serial numbers registered or scanned
- Warranty information and certificates
- Order details and history
- Purchase dates and amounts
- Payment information (processed by third-party payment processor)

**Warranty Claims**:
- Claim descriptions and details
- Photos or documents uploaded for claims
- Resolution history
- Communication records

**Support Communications**:
- Messages sent to customer support
- Feedback and survey responses
- Bug reports and feature requests

### 1.2 Information Collected Automatically

**Device Information**:
- Device type, model, and manufacturer
- Operating system and version
- Unique device identifiers
- Mobile network information
- IP address

**Usage Data**:
- App features accessed and used
- Time and duration of app sessions
- Interaction with app features
- Error logs and crash reports
- Performance data

**Camera Access**:
- QR code scanning (images are processed locally and not stored)
- Photo uploads for warranty claims (stored securely with your permission)

**Location Information** (optional):
- Approximate location (for finding nearby service centers)
- Only collected with your explicit permission
- Can be disabled in app settings at any time

### 1.3 Information from Third Parties

- Verification data from battery manufacturers
- Business information from public databases
- Payment processing information from payment providers
- Authentication data from social login providers (if applicable)

---

## 2. How We Use Your Information

We use the collected information for the following purposes:

### 2.1 Core App Functionality
- Create and maintain your account
- Authenticate your identity
- Process warranty registrations and verifications
- Generate and manage warranty certificates
- Process orders and track shipments
- Allocate and manage inventory
- Enable QR code scanning and verification
- Prevent counterfeiting and fraud

### 2.2 Communications
- Send order confirmations and updates
- Notify you of warranty expirations
- Alert you to important account or service changes
- Respond to your customer support requests
- Send administrative messages and security alerts

### 2.3 Business Operations
- Analyze app usage to improve features
- Generate analytics and reports for business users
- Conduct internal research and development
- Maintain and improve app performance
- Detect and prevent technical issues
- Ensure security and prevent fraud

### 2.4 Marketing (with consent)
- Send promotional offers and updates
- Notify you of new features
- Provide personalized recommendations
- Conduct surveys and market research

You can opt-out of marketing communications at any time through app settings or by contacting us.

### 2.5 Legal Compliance
- Comply with legal obligations and regulations
- Enforce our Terms of Service
- Protect our rights and property
- Respond to legal requests and prevent harm

---

## 3. How We Share Your Information

We do not sell, rent, or trade your personal information to third parties. We may share your information in the following limited circumstances:

### 3.1 With Your Consent
- When you explicitly authorize us to share information
- When you share warranty certificates with others

### 3.2 Service Providers
We share information with trusted third-party service providers who perform services on our behalf:
- Cloud hosting providers (AWS, Google Cloud, etc.)
- Payment processors (for order transactions)
- Email and SMS delivery services (for notifications)
- Customer support platforms
- Analytics providers (anonymized data)

All service providers are contractually obligated to protect your information and use it only for specified purposes.

### 3.3 Business Partners
- Battery manufacturers (for warranty verification)
- Authorized service centers (for warranty claims)
- Logistics partners (for order fulfillment)

Only relevant information is shared for specific transactions.

### 3.4 Legal Requirements
We may disclose information when required by law or in response to:
- Court orders or subpoenas
- Legal processes or government requests
- Protection of our rights, property, or safety
- Fraud prevention or security investigations
- Enforcement of our Terms of Service

### 3.5 Business Transfers
If we are involved in a merger, acquisition, or sale of assets, your information may be transferred. We will notify you before your information is transferred and becomes subject to a different privacy policy.

---

## 4. Data Security

We implement industry-standard security measures to protect your information:

### Technical Safeguards
- **Encryption**: All data transmitted between your device and our servers uses TLS/SSL encryption
- **Secure Storage**: Data at rest is encrypted using AES-256 encryption
- **Authentication**: JWT-based secure authentication with token expiration
- **Access Controls**: Role-based access control limits who can view your data

### Organizational Safeguards
- Regular security audits and vulnerability assessments
- Employee training on data protection
- Limited access to personal information (need-to-know basis)
- Incident response procedures for data breaches

### Physical Safeguards
- Secure data centers with restricted access
- 24/7 monitoring and surveillance
- Redundant backups and disaster recovery plans

**Important**: While we strive to protect your information, no method of transmission or storage is 100% secure. You are responsible for maintaining the confidentiality of your account password.

---

## 5. Your Data Rights

Depending on your location, you may have the following rights:

### 5.1 Access and Portability
- **Right to Access**: Request a copy of your personal information
- **Data Portability**: Receive your data in a structured, machine-readable format

### 5.2 Correction and Deletion
- **Right to Correct**: Update or correct inaccurate information
- **Right to Delete**: Request deletion of your personal information (subject to legal obligations)

### 5.3 Control and Consent
- **Right to Opt-Out**: Unsubscribe from marketing communications
- **Right to Restrict**: Limit how we use your information
- **Right to Object**: Object to processing based on legitimate interests
- **Right to Withdraw Consent**: Withdraw previously given consent

### 5.4 How to Exercise Your Rights
To exercise these rights:
1. Email us at: privacy@lithovolt.com.au
2. Use the in-app data request form (Settings > Privacy > Data Request)
3. Write to us at: [Company Address]

We will respond to your request within 30 days (or as required by local law).

---

## 6. Data Retention

We retain your information only as long as necessary for the purposes outlined in this policy:

- **Active Accounts**: Information retained while your account is active
- **Warranty Records**: Retained for the warranty period plus 7 years (legal requirement)
- **Transaction Records**: Retained for tax and accounting purposes (typically 7 years)
- **Marketing Data**: Deleted immediately upon opt-out request
- **Deleted Accounts**: Personal data deleted within 90 days of account closure (except as required by law)

Anonymized or aggregated data may be retained indefinitely for analytics and research purposes.

---

## 7. Children's Privacy

Lithovolt Battery Manager is not intended for use by individuals under the age of 18 (or the age of majority in your jurisdiction). We do not knowingly collect personal information from children.

If you are a parent or guardian and believe your child has provided us with personal information, please contact us at privacy@lithovolt.com.au, and we will delete the information promptly.

---

## 8. International Data Transfers

Your information may be transferred to and maintained on servers located outside your country of residence, where data protection laws may differ.

If you are located in the European Economic Area (EEA), United Kingdom, or Switzerland:
- We comply with GDPR requirements
- We use standard contractual clauses approved by the European Commission
- We ensure adequate safeguards are in place for international transfers

By using our app, you consent to the transfer of your information to countries outside your jurisdiction.

---

## 9. Specific Regional Privacy Rights

### 9.1 European Economic Area (EEA) - GDPR
- Legal basis for processing: Consent, contract, legitimate interests
- Right to lodge complaint with supervisory authority
- Data Protection Officer contact: dpo@lithovolt.com.au

### 9.2 California - CCPA/CPRA
California residents have additional rights:
- Right to know categories and specific pieces of information collected
- Right to know if information is sold or shared (we do not sell information)
- Right to opt-out of sales (not applicable as we don't sell data)
- Right to non-discrimination for exercising rights

### 9.3 Other Jurisdictions
We comply with local data protection laws including:
- Brazil (LGPD)
- India (DPDP Act)
- Australia (Privacy Act)
- Canada (PIPEDA)

---

## 10. Cookies and Tracking Technologies

Our mobile app does not use traditional browser cookies. However, we use similar technologies:

### 10.1 Local Storage
- Stores authentication tokens securely on your device
- Saves app preferences and settings
- Caches data for offline functionality

### 10.2 Analytics
- Collects anonymized usage statistics
- Helps us understand feature usage and improve the app
- Does not collect personally identifiable information without consent

### 10.3 Push Notifications
- Requires your permission to send notifications
- Can be disabled in device settings
- Used only for order updates, warranty alerts, and important messages

---

## 11. Third-Party Links and Services

Our app may contain links to third-party websites or integrate with third-party services:
- We are not responsible for the privacy practices of third parties
- We encourage you to review their privacy policies
- Third-party services include:
  - Payment processors
  - Map and location services (if applicable)
  - Social media platforms (if sharing is enabled)

---

## 12. Changes to This Privacy Policy

We may update this Privacy Policy periodically to reflect:
- Changes in our practices
- Changes in applicable laws
- New features or services

**Notification of Changes**:
- We will notify you via email or in-app notification
- Significant changes will require your consent
- Continued use after changes constitutes acceptance
- Previous versions will be archived and available upon request

**Last Updated**: This policy was last updated on February 19, 2026. Check the "Last Updated" date at the top to see when changes were made.

---

## 13. Contact Us

If you have questions, concerns, or requests regarding this Privacy Policy or our data practices, please contact us:

**Email**: privacy@lithovolt.com.au  
**Support**: support@lithovolt.com.au  
**Phone**: [Your phone number]

**Mailing Address**:  
Lithovolt Inc.  
[Street Address]  
[City, State/Province, ZIP/Postal Code]  
[Country]

**Data Protection Officer** (for GDPR inquiries):  
Email: dpo@lithovolt.com.au

**Response Time**: We will respond to all inquiries within 30 days (or as required by applicable law).

---

## 14. Consent

By using Lithovolt Battery Manager, you acknowledge that you have read and understood this Privacy Policy and consent to the collection, use, and disclosure of your information as described herein.

**For Specific Consents**:
- Camera access for QR scanning: Granted via device permissions
- Location access: Granted via device permissions (optional)
- Marketing communications: Managed in app settings
- Push notifications: Managed in device settings

You can withdraw consent at any time through the app settings or by contacting us.

---

## Appendix: Data Processing Details

### Categories of Personal Data Processed
| Category | Examples | Purpose |
|----------|----------|---------|
| Identity Data | Name, username, role | Account management |
| Contact Data | Email, phone, address | Communications |
| Transaction Data | Orders, warranties, payments | Service delivery |
| Technical Data | IP address, device info | Performance, security |
| Usage Data | App interactions, features used | Analytics, improvements |
| Marketing Data | Preferences, responses | Personalized communications |

### Legal Basis for Processing (GDPR)
- **Consent**: Marketing communications, optional features
- **Contract**: Account creation, order processing, warranty services
- **Legitimate Interests**: Fraud prevention, analytics, customer support
- **Legal Obligation**: Tax records, warranty compliance, law enforcement requests

---

**End of Privacy Policy**

*This is a template. Please consult with a legal professional to ensure compliance with all applicable laws in your jurisdiction before publishing.*

---

**Document Version**: 1.0  
**Template Created**: February 19, 2026  
**Next Review**: Before app store submission
